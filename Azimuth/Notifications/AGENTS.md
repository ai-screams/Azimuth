...
`GeneralPaneViewController`가 토글을 되돌리고(켜져 보이는데 알림이 안 오는 상태 방지), **beep 없이**
...
## For AI Agents
- 권한 요청 경로는 토글 액션(`GeneralPane+Actions.notifyOnFailureChanged`) 한 곳뿐이어야 한다. 앱 시작/알림 발송 시점에 권한을 요청하지 말 것.
- `GeneralPaneViewController`는 UserNotifications를 import하지 않는다 — `AppDelegate`가 클로저(`requestNotificationAuthorization`)로 주입한다(Sparkle과 같은 결합 회피 패턴).
