...
| `GeneralPaneViewController.swift` | 설정창 **General 탭**(프로퍼티·생명주기·상태 갱신): Permissions/Behavior/Updates 카드. Updates 카드: `versionLabel`(현재 버전 표시) + `checkForUpdatesButton`("Check for Updates…", Sparkle 클로저 호출). 폰트·서브뷰 팩토리는 `GeneralPane+Layout.swift`, `@objc` 액션은 `GeneralPane+Actions.swift`로 분리. 스크롤뷰·스택 뼈대는 `Settings/SettingsPaneScaffold`. 설정창은 툴바 탭 3개(General/Shortcuts/Advanced, `Settings/AGENTS.md`)이고 이 파일은 그중 하나의 페인이다 |
...
| `Settings/` | 설정창: 윈도우 컨트롤러 + 툴바 탭 컨테이너 + Shortcuts/Advanced 페인 + 공용 뼈대 (see `Settings/AGENTS.md`) |
...
| `GeneralPaneViewController.checkForUpdates` | Settings Updates 카드의 버튼 클로저 — `AppDelegate`에서 주입 |
