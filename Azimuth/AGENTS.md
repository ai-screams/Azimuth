...
| `UpdateVersionDisplayer.swift` | `nonisolated` Sparkle adapter (`SPUStandardUserDriverDelegate` + `SUVersionDisplay`) routing the standard update window's version strings through `Shared/UpdateVersionText` ("v1.7.2 Legacy 2 is now available—you have v1.7.2 Legacy 1"). `AppDelegate` retains it and passes it as `userDriverDelegate` (its `updaterDelegate` stays `self` for the feed switch). Checked against real Sparkle by `scripts/sparkle-adapter-check.sh` (CI). Same file as main |
