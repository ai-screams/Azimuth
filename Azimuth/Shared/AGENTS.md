...
| `WindowCommandError.swift` | `nonisolated`. 명령 실행 상위 에러(`resolution`/`workAreaUnavailable`/`notMovable`/`applyFailed`/`noUndoState`/`resolveBudgetExceeded`) + `userFacingMessage` |
| `AXMessagingTimeout.swift` | `nonisolated`. AX messaging timeout 값(`resolve` 0.5초 / `write` 2.0초)과 **둘의 대소 불변식**을 소유한다. 고급 설정용 `clampedResolve`(0·음수·NaN·범위 밖을 안전하게 접는다 — 0은 AX가 "6초 기본값 복귀"로 해석)와 선택지 `ResolveTimeoutChoice`(값+표시 문구를 한곳에 묶어 인덱스 어긋남 방지)도 여기 있다. 적용은 `WindowAccess/FocusedWindowResolver`(해석 진입)와 `WindowFrameWriter`(set 직전)로 나뉘지만 값은 여기 하나뿐이다. 해석 단계 전체 예산 `resolveBudget(for:)` = `resolveReadCount`(6, 표준 창의 AX 왕복 수) × 사용자 상한도 여기 있다 — 고정 상수이던 시절 Patient 선택이 침묵 실패를 늘리던 결함을 없앤다. Foundation만 import하는 순수 파일이라 테스트 하네스에 포함되며, `write >= resolve`와 `resolveBudget(for: resolve) == 3.0`(이전 상수 동일성)을 `make test`가 강제한다(어기면 상향이 상한을 *내리는* 동작이 되는데 컴파일 에러가 안 난다) |
