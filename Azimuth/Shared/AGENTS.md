...
| `BundleVersion.swift` | The `Bundle` extension `displayVersion(prefix:)`. Assembles `CFBundleShortVersionString` (plus the build number) into a display string. Shared by `AboutWindowController` and `GeneralPaneViewController` so the version string is not duplicated |
| `UpdateVersionText.swift` | Pure, harness-tested version text for **Sparkle's update window only**: `label` → `v1.7.2`, `v1.7.2 Legacy N` for a legacy `209.N` build (`legacySequence`); `pair` adds `(build)` to both sides only when the two labels come out identical. About/Settings keep `BundleVersion` ("Version 1.7.2"). Shared verbatim with `legacy/10.13` |
...
| `Log.swift` | `os.Logger` categories (`app`, `windows`) under the subsystem `com.aiscream.Azimuth` |
