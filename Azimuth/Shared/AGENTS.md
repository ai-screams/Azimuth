...
| `UpdateVersionText.swift` | Pure, harness-tested version text for **Sparkle's update window only**: `label` → `v1.7.2`, `v1.7.2 Legacy N` for a legacy `209.N` build (`legacySequence`, also used by `VersionDisplay` so About and the update window share one rule); `pair` adds `(build)` to both sides only when the two labels come out identical. Same file as main |
