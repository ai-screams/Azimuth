...
| `FocusedWindowResolver.swift` | `@MainActor`. 대상 앱의 `kAXFocusedWindowAttribute`를 `ResolvedWindow`로 해석. 앱·창 AX element에 2초 messaging timeout을 설정하고 실패 시 즉시 중단. 권한·풀스크린(비공개 `AXFullScreen`)·최소화·subrole(`kAXStandardWindowSubrole`) 가드와 AX 오류 매핑 |
